...
public class AskOmDchPlaceOrderStepsDefenitions {
    ...
    @And("user clicks on cash on delivery option")
    public void userClicksOnCashOnDeliveryOption() {
        WebElement cashOnDelivery = getDriver().findElement(By.id("payment_method_cod"));
        cashOnDelivery.click();
    }

    @And("user clicks on payment method randomly")
    public void userClicksOnPaymentMethodRandomly() {
        // CASH AND CARD METHODS' XPATHS ARE SAME, ONLY LAST 3 CHARS ARE DIFFRENT. COD IS LAST 3 CHARS FOR CASH METHOD, BACS IS LAST 3 CHAR FOR CARD..
        String[] arr = {"cod", "bacs"};
...
//        WE WILL USE EITHER THIS METHOD TO VERIFY THE CHECKOUT PAGE BUT THE METHOD NEEDS TO BE CORRECTED!
        WebElement cashVerifyMsg = getDriver().findElement(By.xpath("//*[contains(text(),'Cash on delivery')][1]"));
        WebElement cardVerifyMsg = getDriver().findElement(By.xpath("//strong[contains(text(),'Direct bank transfer')]"));
        if(randomPaymentMethod.equals(getDriver().findElement(By.id("payment_method_cod")))){
            Assert.assertEquals("Verify messages do not match",cashVerifyMsg.getText().trim(),"Cash on delivery");
            System.out.println(randomPaymentMethod);
        } else {
            Assert.assertEquals("", cardVerifyMsg.getText().trim(),"Direct bank transfer");
        }
    }

       //OR WE WILL USE THIS METHOD TO VERIFY THE CHECKOUT PAGE BUT THE METHOD NEEDS TO BE CORRECTED! (THERE IS PROBLEM WITH THE XPATHS!)
//    @And("user should be able to see confirmation message")
//    public void userShouldBeAbleToSeeConfirmationMessage() {
//        WebElement verifyMsg = getDriver().findElement(By.xpath("//*[@class='woocommerce-order-overview__payment-method method']/strong"));
//        WebElement cardVerifyMsg = getDriver().findElement(By.xpath("//*[@class=\"woocommerce-order\"]//strong[contains(text(),'Direct bank transfer')]"));
//
//        if(cardVerifyMsg.isDisplayed()){
//            Assert.assertEquals( "Verify messages do not match",cardVerifyMsg.getText().trim(),"Direct bank transfer");
//        }
//        if(cashVerifyMsg.isDisplayed()){
//            Assert.assertEquals("Verify messages do not match",cashVerifyMsg.getText().trim(),"Cash on delivery");
//        }
//    }
}
